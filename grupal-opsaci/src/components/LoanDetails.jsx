...
const LoanDetails = ({ formatAmount, amount, term, cycle }) => {
  let monthlyInterestRate;
  switch (cycle){
    case 1 :
      monthlyInterestRate = 0.0695;
    break;
    case 2 :
      monthlyInterestRate = 0.0675;
    break;
    case 3 :
      monthlyInterestRate = 0.065;
    break;
  };
  const annualInterestRate = monthlyInterestRate * 12;
  const iva = 0.16;
  const weekDays = 7;
  const payments = term * 4;
  const weeklyInterestRate = (annualInterestRate * (1 + iva) * weekDays) / 360;
  const weeklyPayment = (amount * weeklyInterestRate * Math.pow((1 + weeklyInterestRate), payments))/(Math.pow((1 + weeklyInterestRate), payments) - 1);
  const totalPayment = weeklyPayment * payments;
  const paymentPerThousand = (1000 * weeklyInterestRate * Math.pow((1 + weeklyInterestRate), payments))/(Math.pow((1 + weeklyInterestRate), payments) - 1);
  const lifeInsurance = (0.057143 * (amount / 1000) * (term * 30));
  ...
  return (
    <LoanDetailsContainer>
      ...
      <DetailValue>{(monthlyInterestRate * 100).toFixed(2)}%</DetailValue>
      <DetailItem>Pago semanal por cada $1000:</DetailItem>
      <DetailValue>${formatAmount(paymentPerThousand)}</DetailValue>
      <DetailItem>Pago semanal:</DetailItem>
      <DetailValue>${formatAmount(weeklyPayment)}</DetailValue>
